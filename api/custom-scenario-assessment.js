...
const STOP_WORDS = new Set([
  "the", "and", "for", "with", "within", "from", "that", "this", "scenario", "scenarios", "would", "could", "should", "about", "into", "over", "under", "through", "across", "after", "before", "during", "while", "very", "more", "less", "than", "minimal", "managed", "limited", "major", "minor", "likely", "unlikely", "possible", "current", "future", "cold", "hot", "containment", "collapse", "shock", "escalation", "attempt", "attempts", "repeated", "disrupted", "halted", "increase", "increases", "continue", "continues", "expand", "expands", "announced", "placed", "activated"
]);
...
function getWeightedSignals(scenario = {}) {
  if (Array.isArray(scenario.weighted_signals) && scenario.weighted_signals.length) return scenario.weighted_signals.map(parseWeightedSignal).filter(Boolean);
...
function scrubImplementationDetails(value, fallback = "Not enough evidence found to support this scenario.") {
  ...
  if (banned.some((pattern) => pattern.test(raw))) return fallback;
  return raw;
}
...
function buildTavilyQueryChain({ scenarios }) {
  ...
  const topicQuery = topicTerms.slice(0, 6).join(" OR ");
  const compactTopic = compactKeywordQuery(topicTerms, 10);
  const signalQuery = signalTerms.slice(0, 8).join(" OR ");
  const compactSignal = compactKeywordQuery(signalTerms, 10);
  return unique([topicQuery, compactTopic, signalQuery, compactSignal, buildScenarioSearchQuery({ scenarios })]).map((query) => cleanText(query, 280)).filter(Boolean);
}
...
function hasScenarioTopicOverlap(article, scenarios) {
  ...
  const tokens = new Set([...scenarioTopicText(scenarios), ...scenarioSignalText(scenarios)].flatMap(tokenise).filter((token) => token.length > 3 && !STOP_WORDS.has(token)));
...
function isWeakEvidencePage(article) {
  ...
  return /\/sitemap\//.test(url) || /\/tag\//.test(url) || /\/tags\//.test(url) || /\/category\//.test(url) || /\/search\?/.test(url) || /home \|/.test(title) || title === "home" || title.includes("schedule") || title.includes("episode guide");
}
...
function aliasSignalMatch(signalText, haystack) {
  ...
  if (signal.includes("military-protected") || signal.includes("protected shipping")) return textHas(h, /(escort|patrol|protect|secure|warship|navy|naval|convoy)/) && textHas(h, /(shipping|tanker|vessel|strait|hormuz|gulf)/);
  ...
  if (signal.includes("red sea shipping")) return textHas(h, /red sea/) || (textHas(h, /(shipping|vessel|tanker|sea)/) && textHas(h, /(disrupt|attack|target|rerout|suspend|halt)/));
  ...
  if (signal.includes("insurance") || signal.includes("freight rates")) return textHas(h, /(insurance|freight|shipping cost|rates|longer route|more expensive|rerout)/) && textHas(h, /(gulf|hormuz|shipping|tanker|vessel|route)/);
  ...
  if (signal.includes("energy infrastructure")) return textHas(h, /(oil facility|energy infrastructure|refinery|pipeline|power plant|production facility|defence|defenses|alert)/);
...
function scoreItemAgainstSignals(item, scenario) {
  ...
  for (const signal of getWeightedSignals(scenario)) {
    ...
    const strictTokenMatch = tokens.length ? hits >= Math.max(2, Math.ceil(tokens.length * 0.65)) : false;
...
function getScenarioContextTokens(scenario = {}) {
  return [scenario.name, scenario.description].flatMap(tokenise).filter((token) => token.length > 3 && !STOP_WORDS.has(token));
}
...
function selectArticlesForModel(articles, scenarios) {
  ...
  return scored.filter((article) => article.total_signal_score > 0).sort((a, b) => b.total_signal_score - a.total_signal_score || String(b.published_at || "").localeCompare(String(a.published_at || ""))).slice(0, MAX_ITEMS_FOR_MODEL);
}
...
function computeScoreCalibrationForScenario(articles, scenario, sources) {
  ...
  const calibratedLikelihood = matchedSignals.length ? clampPercent((weightedSignalCoverage * 0.9) + (sourceSupport * 0.05) + (sourceDiversity * 0.05)) : 0;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return sendJson(res, 200, {
      ...
      source_diagnostics: { synonym_signal_matching_enabled: true, weak_evidence_filter_enabled: true, deterministic_score_calibration_enabled: true, topic_first_query_enabled: true, signal_weights_enabled: true, source_domain_gate_enabled: true, rss_source_count: rssSources.length, tavily_source_count: tavilySources.length, total_articles_collected: articles.length, evidence_articles_after_filter: modelArticles.length, tavily_articles_collected: tavilyArticles.length, tavily_evidence_after_filter: modelArticles.filter((article) => article.searchProvider === "tavily").length, tavily_query_attempts: tavilySources.map((source) => ({ name: source.name, domain: source.domain, attempts: tavilyArticles.find((article) => article.source_id === source.id)?.tavilyQueryAttempts || [] })) }
    });
  } catch (error) {
