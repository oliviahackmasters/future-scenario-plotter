...
async function fetchGoogleNewsQuery(query, searchPlan = null) {
  const cleanQuery = cleanText(query, 180);
  if (!cleanQuery) return [];

  const url = `https://news.google.com/rss/search?q=${encodeURIComponent(cleanQuery)}&hl=en-GB&gl=GB&ceid=GB:en`;

  try {
    const items = await fetchFeed({
      id: `google-news-query-${cleanQuery.slice(0, 30)}`,
    ...
    return items.map((item) => ({
      ...item,
      source_id: "google-news-query",
      source: "Google News Search",
      source_url: url,
      searchProvider: "google-news-rss"
    }));
  } catch (error) {
    console.error("[Google News fallback] failed:", cleanQuery, error.message);
    return [];
  }
}
...
function buildTavilyQueryChain({ scenarios }) {
  const scenarioQueries = scenarios.flatMap((scenario) => {
    ...
    const coreQueries = [
      ...
      `${scenario.name} ${signalTerms.slice(0, 12).join(" ")}`,
...
async function fetchCustomTavilyArticles({ source, scenarios, searchPlan, tavilyDebug = [] }) {
  ...
  const queries = unique([
    ...
    .map((query) => cleanText(query, 280))
    .filter(Boolean)
    .slice(0, isGeneralWebSource(source) ? 5 : 2);
...
function isTooOldForEvidence(article) {
  ...
  // For Tavily (general web search), a null date means the page has no
  // parseable publish date -- common for EU observatory pages, PDF reports,
  // and institutional sites. Treat as age-unknown rather than too-old.
  // For RSS, every real feed item carries a pubDate field, so null
  // genuinely indicates something broken or very old -- keep rejecting.
  if (age === null) {
    const provider = String(article.searchProvider || article.provider || "").toLowerCase();
...
function articleFreshnessMultiplier(article) {
  ...
  // Same provider-aware null handling as isTooOldForEvidence: Tavily articles
  // with no parseable date get a modest but non-zero multiplier (0.4, the same
  // as "recent but not fresh") rather than 0, so they can still contribute
  // evidence when entity/signal matching is strong.
  if (age === null) {
    const provider = String(article.searchProvider || article.provider || "").toLowerCase();
    return provider === "tavily" ? 0.4 : 0;
  }
...
function dedupeArticles(articles) {
  ...
  for (const article of articles) {
    ...
    if (isWeakEvidencePage(article) && provider !== "tavily") continue;
    if (isTooOldForEvidence(article) && provider !== "tavily") continue;
...
if (scenarioName.includes("creative") || scenarioName.includes("fashion")) {
  const fashionContext = /(fashion|luxury|gucci|lvmh|dior|prada|burberry|zara|shein|lookbook|runway|apparel|designer)/.test(haystack);
  const aiContext = /(generative ai|ai-generated|synthetic influencer|synthetic media|artificial intelligence|adobe firefly|midjourney|runway|openai)/.test(haystack);
  const legalContext = /(copyright|intellectual property|lawsuit|infringement|right-of-publicity|trademark|labeling|transparency)/.test(haystack);

  return fashionContext && (aiContext || legalContext) && (entityHits >= 1 || signalHits >= 2);
}
...
async function selectArticlesForModel(articles, scenarios, searchPlan = null) {
  ...
  const rejectedTitles = scoredArticles
    ...
    .map((article) => ({
      ...
      freshness_multiplier: article.freshness_multiplier,
      scenario_matches: article.scenario_matches || [],
      rejected_reason: "total_signal_score <= 0"
    }));

  console.log("ARTICLE FILTER DEBUG", {
    input_count: articles.length,
    scored_count: scoredArticles.length,
    selected_count: scoredArticles.filter((article) => article.total_signal_score > 0).length,
...
const tavilySources = sources.filter((source) =>
  isTavilySource(source) && isGeneralWebSource(source)
);
...
const tavilyArticles = tavilyResults
  .flatMap((result) => result.status === "fulfilled" ? result.value : []);

const tavilyArticleCount = tavilyArticles.length;

const googleNewsFallbackResults = tavilyArticleCount === 0
  ? await Promise.allSettled(
      (searchPlan.search_queries || [])
        .slice(0, 8)
        .map((query) => fetchGoogleNewsQuery(query, searchPlan))
    )
  : [];

const googleNewsFallbackArticles = googleNewsFallbackResults
  .flatMap((result) => result.status === "fulfilled" ? result.value : []);


  const allArticles = [
    ...rssResults,
    { status: "fulfilled", value: tavilyArticles },
    { status: "fulfilled", value: googleNewsFallbackArticles }
  ]
    .flatMap((result) => result.status === "fulfilled" ? result.value : []);
