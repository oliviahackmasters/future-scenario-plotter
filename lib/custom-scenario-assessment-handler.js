...
// Thresholds for sufficient evidence
...
function buildTavilyQueryChain({ scenarios }) {
  const scenarioQueries = scenarios.flatMap((scenario) => {
    ...
    // Expand queries into evidence buckets for broader coverage
    const evidenceBuckets = [];

    // Build buckets from signals - these are the actual search queries
    for (let i = 0; i < signalTerms.length; i += 2) {
      const chunk = signalTerms.slice(i, i + 3);
      if (chunk.length) {
        evidenceBuckets.push(chunk.join(" OR "));
      }
    }

    // Add scenario + context combinations for broader evidence collection
...
async function buildScenarioSearchPlan({ scenarios }) {
  const fallback = {
    search_queries: buildTavilyQueryChain({ scenarios }),
...
- Avoid generic words such as future, scenario, disruption, risk, change, uncertainty unless paired with concrete terms.`
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error("OpenAI search planner timed out")), 12000))
    ]);
...
async function fetchCustomTavilyArticles({ source, scenarios, searchPlan }) {
...
const matchedWeight = getWeightedSignals(scenario).reduce((sum, signal) => {
  ...
  // Check if we have enough evidence to generate a meaningful forecast
  ...
  if (!sufficientEvidence) {
    // Insufficient evidence: return null likelihood with low confidence
...
export default async function handler(req, res) {
  ...
  const tavilyResults = await Promise.allSettled(
    sources
      ...
      .map((source) => fetchCustomTavilyArticles({ source, scenarios, searchPlan }))
  );
