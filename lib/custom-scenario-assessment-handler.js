...
function isGeneralWebSource(source = {}) {
  ...
  return type === "web" || id === "web-search" || id === "general-web" || id === "tavily-web";
}
...
function fallbackStrategicQueries(scenarios = []) {
  ...
  if (/(fashion|luxury|brand|campaign|lookbook|designer|creative)/.test(text) && /(ai|generative|synthetic)/.test(text)) {
    queries.push(
...
export default async function handler(req, res) {
  ...
  return sendJson(res, 200, {
    ...
    debug: {
      ...
      tavily_attempts: allArticles
        .filter((article) => article.searchProvider === "tavily")
        .slice(0, 8)
        .map((article) => ({
          source: article.source,
          title: article.title,
          attempts: article.tavilyQueryAttempts
        })),
